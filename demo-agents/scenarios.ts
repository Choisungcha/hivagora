import { HivagoraAgentSDK } from '../sdk/src/index';
...
async function runScenarios() {
  ...
  hotelAgent.on('message', async (msg: any) => {
    if (msg.type === 'broadcast' && msg.content.goal?.toLowerCase().includes('tokyo')) {
  ...
  travelAgent.on('message', async (msg: any) => {
  ...
  sellerAgent.on('message', (msg: any) => {
  ...
  foodieAgent.on('message', (msg: any) => {
    if (msg.type === 'negotiate') {
